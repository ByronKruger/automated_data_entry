...
def slide_windows(filename, anchor_box, stride=1):
	...
	for hori_step in range(hori_window_steps):
		for vert_step in range(vert_window_steps):
			poss_object = poss_objects[vert_step * anchor_box[1]: vert_step * anchor_box[1] + anchor_box[1],\
									   hori_step * anchor_box[0]: hori_step * anchor_box[0] + anchor_box[0]]
...
if __name__ == "__main__":
	filename = "/home/charlie/Desktop/yolo/yolo-v3/data/coco/cocoapi/coco/images/train2017/train2017/000000001756.jpg"
	anchor_box = []
	anchor_box.append(50) # w
	anchor_box.append(100) # h

	hog_windows = slide_windows(filename, anchor_box, 2)

	train_and_predict_multiclass(['orange', 'bicycle', 'motorbike', 'person'], '200', '7', hog_windows)